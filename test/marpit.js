...
describe('Marpit', () => {
  ...
  describe('#use', () => {
    ...
    context('with PostCSS plugin', () => {
      it('applies simple PostCSS plugin transformation to style output', () => {
        const generateInstance = () => {
          const marpit = new Marpit()

          marpit.themeSet.add(
            '/* @theme x */ section { border: dashed #f00 8px; }',
          )
          return marpit
        }

        const instance = generateInstance()
        expect(instance.use(postcssOrderedValues)).toBe(instance)

        const md =
          '<!-- theme: x -->\n<style>h1{ border: solid 1px red; }</style>'

        const { css: pureCSS } = generateInstance().render(md)
        const { css: extendedCSS } = instance.render(md)

        expect(pureCSS).toContain('border: solid 1px red;')
        expect(pureCSS).toContain('border: dashed #f00 8px;')
        expect(extendedCSS).not.toContain('border: solid 1px red;')
        expect(extendedCSS).not.toContain('border: dashed #f00 8px;')
        expect(extendedCSS).toContain('border: 1px solid red;')
        expect(extendedCSS).toContain('border: 8px dashed #f00;')
      })

      it('applies PostCSS plugin transformation with options to style output', () => {
        const generateInstance = () => {
          const marpit = new Marpit()

          marpit.themeSet.add(
            `/* @theme x */ section::before { content: "\\"hello\\" world"; }`,
          )
          return marpit
        }
        ...
        const instance = generateInstance()
        expect(
          instance.use(postcssNormalizeString, { preferredQuote: 'single' }),
        ).toBe(instance)

        const md = `<!-- theme: x -->\n<style>h1::before { content: "test \\"test\\""; }</style>`
        const { css: pureCSS } = generateInstance().render(md)
        const { css: extendedCSS } = instance.render(md)

        expect(pureCSS).toContain(`content: "\\"hello\\" world";`)
        expect(pureCSS).toContain(`content: "test \\"test\\"";`)
        expect(extendedCSS).not.toContain(`content: "\\"hello\\" world";`)
        expect(extendedCSS).not.toContain(`content: "test \\"test\\"";`)
        expect(extendedCSS).toContain(`content: '"hello" world';`)
        expect(extendedCSS).toContain(`content: 'test "test"';`)
      })

      it('applies PostCSS plugin transformation generated by options to style output', () => {
        const generateInstance = () => {
          const marpit = new Marpit()
        ...
        const instance = generateInstance()
        const generatedPlugin = postcssNormalizeString({
          preferredQuote: 'single',
        })

        expect(instance.use(generatedPlugin)).toBe(instance)
